...
const Dashboard = () => {
    ...
    const handleStatusChange = async (id, newStatus) => {
        try {
            const response = await fetch(`${import.meta.env.VITE_API_BACKEND_URL}/api/contacto/${id}`, {
                method: 'PATCH',
                headers: {
    ...
    const handleBulkDelete = async () => {
        ...
        if (window.confirm(`¿Eliminar ${selectedContacts.length} contactos seleccionados?`)) {
            try {
                const response = await fetch(import.meta.env.VITE_API_BACKEND_URL + '/api/contacto/bulk', {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json',
                    },
